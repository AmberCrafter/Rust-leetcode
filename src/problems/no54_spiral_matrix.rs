    ...
    pub fn spiral_order(matrix: Vec<Vec<i32>>) -> Vec<i32> {
        ...
        while (r>=l) && (b>=t) && (!outofbound) {
            match step%4 {
                0 => {
                    for _i in l..=r {
                        if taggle {h+=1} else {taggle = true}                
                        res.push(matrix[v][h]);
                    }
                    t+=1;
                }
                ...
                1 => {
                    ...
                    if r>0 {r-=1} else {outofbound=true}
                }

                2 => {
                    for _i in l..=r {
                        if h>0 {h-=1} else {outofbound=true}
                        res.push(matrix[v][h]);
                    }
                    if b>0 {b-=1} else {outofbound=true}
                }

                3 => {
                    for _i in t..=b {
                        if v>0 {v-=1} else {outofbound=true}
                        res.push(matrix[v][h]);
                    }
                    l+=1;
                }
...
mod tests {
    ...
    #[test]
    fn case_2() {
        let inputs = [[1,2,3,4].to_vec(),[5,6,7,8].to_vec(),[9,10,11,12].to_vec()].to_vec();
